...
% optional output for keeping track of number of each reactions
if nargout==5
   reaction_label_arr = []; % aggregation, fragmentation, expulsion
elseif nargout~=4
    disp(['error in simulate_cluster_gillespie: num output args must be 4 or 5']);
    return;
end
%% fixed parameters
...
while t < Tmax 
    ...
    if ~isempty(reaction_id)
        ...
        if nargout==5
            if reaction_id > 0
                this_label_tmp = all_labels(reaction_id);
            else
                this_label_tmp  = 0;
            end
            reaction_label_arr = [reaction_label_arr; [t+delta_t, this_label_tmp]];
        end
    end
    ...
    if nargout==5
        varargout{1} = reaction_label_arr;
    end
